    ...
    class SkyboxGenerator
    {
        public static SkyBox Generate(Texture2D horizon, float repetionsX, DepthCueMode depthCueMode)
        {
            if (horizon == null)
            {
                ...
                horizon = new Texture2D(GameEngine.Device, 1, 1, false, SurfaceFormat.Color);
            ...
            Texture2D topTexture = new Texture2D(GameEngine.Device, 1, 1, false, SurfaceFormat.Color);
            topTexture.SetData<Color>(new Color[] { pixels[0] });  //top left pixel

            Texture2D bottomTexture = new Texture2D(GameEngine.Device, 1, 1, false, SurfaceFormat.Color);
            bottomTexture.SetData<Color>(new Color[] { pixels[pixels.Length - 1] }); //bottom right pixel

            Texture2D sideTexture = new Texture2D(GameEngine.Device, horizon.Width, horizon.Height, false, SurfaceFormat.Color);
            int ptr = 0;
