			...
			while (true)
			{
				...
				switch (blockType)
				{
					...
					case PixBlockType.PixelData:
                        ...
                        if (bytesPerPixel == 1)
                        {
                            texture = new Texture2D(GameEngine.Device, currentPix.Width, currentPix.Height, 1, TextureUsage.None, SurfaceFormat.Color);
                            texture.SetData<byte>(Helpers.GetBytesForImage(pixels, currentPix.Width, currentPix.Height, GameVars.Palette));
                        }
                        else if (bytesPerPixel == 2)
                        {
                            texture = new Texture2D(GameEngine.Device, currentPix.Width, currentPix.Height, 1, TextureUsage.None, SurfaceFormat.Bgr565);
                            int j = 0;
                            byte[] px = new byte[2];
